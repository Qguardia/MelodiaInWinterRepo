...
public class ViewzoneSentry : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        sightRange = parent.GetComponent<NavMeshAgentSentry>().sightRange;
        lightChecker = GameObject.Find("lightChecker");
    }

    private void FixedUpdate()
    {
        if (parent.gameObject.GetComponent<NavMeshAgentSentry>().AIState < 3)
        {
            RayCastCheck();
            ...
            if (inLOS == true)
            {
                ...
                parent.gameObject.GetComponent<NavMeshAgentSentry>().AIState = 1; // HEAD TOWARDS PLAYER
                enemyGun.TryToFireGun();
            }
            else
            {
                //enemyGun.TryToFireGun();
                parent.gameObject.GetComponent<NavMeshAgentSentry>().AIState = 4;
            }
        }
    }
    ...
    private void OnTriggerExit(Collider other)
    {
        if (other.gameObject.tag == "PlayerBody")
        {
            ...
            if (parent.gameObject.GetComponent<NavMeshAgentSentry>().AIState == 1)
            {
                parent.gameObject.GetComponent<NavMeshAgentSentry>().AIState = 4;
            }
        }
